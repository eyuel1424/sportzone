...
const GLOBAL_STYLES = `
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .hero-live-card { background:#0f1420; border:1px solid rgba(255,59,59,0.3); border-radius:14px; padding:14px 16px; min-width:220px; max-width:260px; flex-shrink:0; cursor:pointer; transition:transform 0.2s ease,border-color 0.2s ease; animation:livePulse 2.5s ease-in-out infinite; }
...
function GameDetail({ game, sport, onClose }) {
  ...
  return (
    <div onClick={onClose} style={{ position:"fixed", inset:0, zIndex:200, background:"rgba(0,0,0,0.78)", backdropFilter:"blur(8px)", display:"flex", alignItems:"center", justifyContent:"center", padding:20 }}>
      <div onClick={e=>e.stopPropagation()} style={{ background:"#0f1420", border:"1px solid #1e2535", borderRadius:20, width:"100%", maxWidth:520, maxHeight:"90vh", overflowY:"auto", padding:28, display:"flex", flexDirection:"column", gap:22, animation:"fadeUp 0.25s ease both" }}>
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start" }}>
          ...
          <button onClick={onClose} style={{ background:"#141820", border:"1px solid #1e2535", borderRadius:8, width:32, height:32, cursor:"pointer", color:"#8898aa", fontSize:16, display:"flex", alignItems:"center", justifyContent:"center" }}>✕</button>
        </div>
...
function HeroSection({ liveGames, onGameClick }) {
  const totalLive = liveGames.length;
  return (
    <div style={{ padding:"44px 20px 32px", maxWidth:1100, margin:"0 auto", animation:"heroFade 0.5s ease both" }}>
      <div style={{ marginBottom: totalLive>0?28:0 }}>
        ...
        <h1 style={{ fontFamily:"'Bebas Neue',sans-serif", fontSize:"clamp(44px,7vw,82px)", letterSpacing:"3px", color:"#f0f0f0", lineHeight:0.93, marginBottom:14 }}>
          Never Miss<br />
        ...
        <p style={{ fontSize:15, color:"#5a6478", maxWidth:460, lineHeight:1.65 }}>
                  ...
                  <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", gap:8 }}>
                    {[away,home].map((c,ci)=>(
                      <div key={ci} style={{ display:"flex", flexDirection:"column", alignItems:"center", gap:3, flex:1 }}>
                        {c?.team?.logo&&<img src={c.team.logo} alt="" style={{ width:28,height:28,objectFit:"contain" }} onError={e=>e.target.style.display="none"} />}
                        ...
                        <span style={{ fontSize:26, fontWeight:800, fontFamily:"'Bebas Neue',sans-serif", color:"#f0f0f0" }}>{c?.score??"-"}</span>
                      </div>
                    ))}
                  </div>
                </div>
...
export default function SportZone() {
  ...
  const fetchLiveAll = useCallback(async () => {
    try {
  ...
  useEffect(() => { fetchLiveAll(); }, []);

  useEffect(() => {
    if (timerRef.current) clearInterval(timerRef.current);
    timerRef.current = setInterval(() => {
      fetchAllDays(activeSport, true);
      fetchLiveAll();
    }, REFRESH_MS);
    return () => clearInterval(timerRef.current);
  }, [activeSport]);

  const handleRefresh = () => { fetchAllDays(activeSport, false); fetchLiveAll(); };
  ...
  return (
    <>
        ...
        <div style={{ maxWidth:1100, margin:"0 auto", padding:"0 20px" }}>
          <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"14px 0 10px" }}>
            ...
            <div style={{ display:"flex", alignItems:"center", gap:12 }}>
              {lastUpdated&&<span style={{ fontSize:11, color:"#3a4255" }}>Updated {lastUpdated.toLocaleTimeString("en-US",{hour:"numeric",minute:"2-digit"})}</span>}
          ...
          {/* Sport tabs — wraps on small screens */}
          <div className="tabs-wrap">
            {SPORTS.map(s=>{
              const active = activeSport.id===s.id;
              const hasLive = liveGames.some(g=>g._sport?.id===s.id);
              return (
                <button key={s.id} className="sport-btn"
                  onClick={()=>{ setActiveSport(s); setActiveDay("today"); }}
                  style={{ background:active?"rgba(99,179,237,0.1)":"none", border:"none", padding:"8px 13px", borderRadius:"8px 8px 0 0", fontSize:13, fontWeight:active?700:500, color:active?"#63b3ed":"#5a6478", whiteSpace:"nowrap", borderBottom:active?"2px solid #63b3ed":"2px solid transparent", fontFamily:"'DM Sans',sans-serif", position:"relative" }}>
                  {s.emoji} {s.label}
                  {hasLive&&<span style={{ position:"absolute", top:6, right:4, width:6, height:6, borderRadius:"50%", background:"#ff3b3b", animation:"pulse 1.2s ease-in-out infinite" }} />}
                </button>
              );
            })}
          </div>
      ...
      {/* Hero */}
      <HeroSection liveGames={liveGames} onGameClick={openGame} />

      {/* Divider */}
      <div style={{ maxWidth:1100, margin:"0 auto", padding:"0 20px" }}>
        <div style={{ borderTop:"1px solid #141820", marginBottom:28 }} />
      </div>
      ...
      <div style={{ maxWidth:1100, margin:"0 auto", padding:"0 20px 60px" }}>
        ...
        {!loading&&!error&&activeDay!=="week"&&displayGames.length===0&&
          <div style={{ textAlign:"center", padding:"80px 0" }}>
            <div style={{ fontSize:52, marginBottom:16 }}>{activeSport.emoji}</div>
            <div style={{ fontSize:20, fontWeight:700, color:"#f0f0f0", marginBottom:8, fontFamily:"'Bebas Neue',sans-serif", letterSpacing:"1px" }}>
              No {activeSport.label} games {activeDay==="today"?"today":"tomorrow"}
            </div>
