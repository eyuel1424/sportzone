...
export default function SportZone() {
  ...
  const fetchDay = useCallback(async (sport, dateStr) => {
    ...
    // Filter out games not from the requested date
  ...
  const fetchLiveAll = useCallback(async (isFirst=false) => {
    ...
    try {
      ...
      const results=await Promise.all(SPORTS.map(async sport=>{
        ...
        // Only include games from today — strictly filter by date AND exclude old completed games
        const todayEvents = events.filter(g=>{
          const gameKey = getDateKey(g.date);
          const isToday = gameKey === todayKey;
          ...
          // If completed/final and not today, definitely exclude
          if ((completed || isFinal) && !isToday) return false;
          return isToday;
        });
